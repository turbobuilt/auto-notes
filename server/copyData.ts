    ...
    return callMethod("${methodIdentifier}", [...arguments], ${
        ...
        for (let exported of exports) {
            ...
            if (exported[1][0] instanceof ClassDeclaration) {
                console.log("Copying class", exported[0], "in file", sourceFile.getFilePath())
                ...
                if (extendsStr && extendsInfo) {
                    console.log("got extends", extendsStr,declaration.getExtends()?.getText())
...
function importType(type: Type, project: Project, currentFile: SourceFile, outputFile: SourceFile) {
    ...
    console.log("importing", fullTypeStr)
