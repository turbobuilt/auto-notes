...
interface Progress {
   onLoadProgress: (loadingPercent: number) => void;
}

export async function* summarizeTranscript(transcript: string, progress: Progress): AsyncGenerator<string, string, unknown> {
    const { CreateMLCEngine, MLCEngine } = llm;
    
    // Initialize with the provided progress callback
    const initProgressCallback = (loadingProgress: number) => {
        console.log("Model loading progress:", loadingProgress);
        progress.onLoadProgress(loadingProgress);
    };
...
Once you receive the transcript, use these points to produce a detailed and organized note.`
    ...
    ] as llm.ChatCompletionMessageParam[];
    ...
    for await (const chunk of chunks) {
        ...
        // Yield each chunk so the caller can process it
        yield reply;
    }

    // Return the full reply as the final value
    return fullResponse;
}